...
// Task 3
// При нажатии b-3 выполняете функцию f3. Функция должна вывести в out-3 значения hi из массива a3.
// Возвратите результирующую строку значение. Вывод - через пробел.
// Вывод, как и в предыдущем задании пишем "руками"
...
function f18() {
  let metroStation = document.querySelector('.i-18').value;
  let out = '';
  for (let key in a18) {
    if (metroStation === key) {
      out += a18[key].join(' ') + ' ';
  ...
  document.querySelector('.out-18').innerHTML = out.trim();
}
...
function f19() {
  let lineColor = document.querySelector('.i-19').value.toLowerCase();
  let out = '';
  for (let color in a19) {
    for (let station of a19[color]) {
      if (station.toLowerCase() === lineColor) {
        out = color;
      }
    }
  }
...
function f20() {
  let stationsWithTransfer = [];

  for (let line in a20) {
    for (let station of a20[line]) {
      if (station[1] === 2) {
        stationsWithTransfer.push(station[0]);
      }
    }
  }

  document.querySelector('.out-20').textContent =
    stationsWithTransfer.join(' ');
}
