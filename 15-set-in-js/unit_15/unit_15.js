...
const f7 = () => {
  ...
  if (password.size === input.length && input.length > 6) {
    out.textContent = 1;
  } else {
    out.textContent = 0;
  }
...
const f9 = (our_set) => {
  const myString = Array.from(our_set).join(' ');
  return myString;
};
...
const f10 = (out_set, elem) => {
  let newSet = Array.from(out_set).join(' ');
  document.querySelector(elem).textContent = newSet;
};
...
// Task 13
// При нажатии b-13 выполняете функцию f13. Функция должна посчитать, сколько каждый символ входит в строку.
// Результат в виде объекта (ключи - символы, значения - сколько символ встречается в строке) возвратить.
